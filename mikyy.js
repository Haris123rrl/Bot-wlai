...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const getCase = (cases) => {
            return "case  "+`'${cases}'`+fs.readFileSync("./mikyy.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
        }
        const totalFitur = () =>{
            var mytext = fs.readFileSync("./mikyy.js").toString()
        ...
        // Function Created By MikyModzz Alias Keyraa cantik
...
break
            case 'sc':
            case 'script':
               newReply(`Script Bot ${namabot} https://youtube.com/@mikyoffc`)
            break
        // Group Fitur
            case 'antilinkv2':
                if (!m.isGroup) return newReply(mess.group)
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
		...
		case 'brainly':{
            ...
            case 'attp':
                ...
                newReply(`🚩 5 Limit Used`)
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} MikyModzz`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By MikyModzz.mp3`}, { quoted : m })
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By MikyModzz\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} MikyModzz `)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By MikyModzzz\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'carvedwood':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} MikyModzz`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By MikyModzzz\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} MikyModzz`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By MikyModzzz\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'thunder':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} MikyModzz`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By MikyModzz\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
...
◇ ‣ ${prefix}memeindo`
        ...
        console.log("Eror Di Bagian mikyy.js "+util.format(err))
    }
}
